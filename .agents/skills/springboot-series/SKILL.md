...
- **Página de Coleção / Trilha da Série:** A série possui sua página dedicada em [`tutoriais/spring-boot.md`](tutoriais/spring-boot.md) (`/tutoriais/spring-boot/`) baseada no layout `serie`. Os metadados da série e a ementa de capítulos futuros estão centralizados em [`_data/series.yml`](_data/series.yml). Novos capítulos publicados são incluídos automaticamente na trilha e devem manter paridade com o roteiro de 17 partes.
...
Consulte o arquivo [`ROTEIRO.md`](ROTEIRO.md) para a ementa completa das 15 partes.

Cada post deve seguir o arco narrativo definido na skill [`blog-posts`](SKILL.md):
