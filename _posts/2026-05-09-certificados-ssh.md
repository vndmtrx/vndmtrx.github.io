...
> Essa gerência do ciclo de vida dos usuários e de suas permissões granulares de elevação de privilégio é um assunto tão extenso e vital que com certeza faremos um post dedicado só para isso no futuro.
...
> [!NOTE] Nota
> A variável `%u` é parseada como o nome do usuário de destino durante o login.
...
> [!NOTE] Nota
> Esse usuário `db_admin` não é o dono do banco. Ele loga via SSH e depois usa privilégios no `/etc/sudoers` para rodar um `sudo -u postgres psql` ou `sudo systemctl restart postgresql`, mantendo a trilha de auditoria limpa.
