...
subtitle: "Anatomia do CVE-2026-31431: corrupção de page cache no kernel e escalada de privilégios via AF_ALG"
author:
- "Eduardo N. S. R."
...
Mas o exploit funciona em qualquer distro com kernel afetado. Eu testei nas seguintes e até a data dessa análise eram considerados vulneráveis: Debian, Archlinux, Fedora, Rocky Linux, AlmaLinux, OpenSUSE e Oracle Linux. Basicamente, se o kernel foi compilado entre 2017 e o patch, está no escopo desse exploit, conforme registrado nos boletins e análises oficiais [^8] [^9] [^10] [^11] [^12].

## Análise do Código do Exploit

> [!NOTE] Disclaimer
> Toda essa análise só foi possível graças ao trabalho extremamente detalhado da equipe do [Xint/Theori](https://xint.io/blog/copy-fail-linux-distributions), que explica a causa raiz, o mecanismo da falha e o fluxo do exploit com uma clareza impressionante. Além disso, também me apoiei em análises complementares da [NVD](https://nvd.nist.gov/vuln/detail/CVE-2026-31431), [R-fx Networks](https://www.rfxn.com/research/copyfail-cve-2026-31431), [ToolsLib](https://blog.toolslib.net/2026/04/30/cve-2026-31431-copy-fail-what-you-need-to-know/), [Tenable](https://www.tenable.com/blog/copy-fail-cve-2026-31431-frequently-asked-questions-about-linux-kernel-privilege-escalation) e do [Debian Security Tracker](https://security-tracker.debian.org/tracker/CVE-2026-31431) para entender mais sobre o problema e para algumas partes como a análise do asm do exploit. O que faço aqui é dar um detalhamento mais mastigado do código Python para quem não quer (ou não tem tempo de) mergulhar no exploit por conta própria. Dito isso, recomendo fortemente que vocês leiam o post original e tentem destrinchar o código sozinhos, é um exercício excelente de análise de segurança.

Agora vamos ao que interessa: o código completo do exploit [^3]. São poucas linhas de código Python:

<script src="https://gist.github.com/vndmtrx/24befcc16625aca6d6a7e95e1929fb16.js"></script>
...
i=0
e=zlib.decompress(d("78daab77f571...32c310d3"))
```
...
b'\x7fELF\x02\x01\x01\x00...[bytes_do_shellcode_omitidos]...
/bin/sh\x00\x00\x00'
```
...
**Atualização (08/05/2026):** O mais preocupante é que o Copy Fail parece ter aberto a porteira para toda uma nova classe de ataques. Uma vez que a técnica de usar APIs de rede e `splice()` para corromper áreas protegidas do *page cache* tornou-se pública, outros pesquisadores começaram a buscar falhas semelhantes. E encontraram. Em pouco tempo, surgiram novos exploits na mesma "estirpe", como o **Dirty Frag** [^13] (que encadeia falhas de corrompimento de *page cache* no xfrm-ESP e no RxRPC para obter root) e o **Copy Fail 2: Electric Boogaloo** [^14] (que explora o *fast path* no-COW do xfrm ESP-in-UDP). Isso evidencia que o que a análise da IA expôs não foi apenas um bug isolado, mas uma nova superfície de ataque estrutural no kernel Linux, baseada na interseção de APIs complexas que acabam manipulando o cache de forma indevida.
...
[^7]: **Linux e LLMs - Não é melhor legalizar que fingir que não existe?** {*Vndmtrx, 11/04/2026*} ({% include post-ref.html slug="linux-llm" text="Link" %})
...
[^13]: **Dirty Frag (CVE-2026-43284 / CVE-2026-43500)** {*V4bel / GitHub*} ([Link](https://github.com/V4bel/dirtyfrag))

[^14]: **Copy Fail 2: Electric Boogaloo** {*0xdeadbeefnetwork / GitHub*} ([Link](https://github.com/0xdeadbeefnetwork/Copy_Fail2-Electric_Boogaloo))
